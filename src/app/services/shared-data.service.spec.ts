...
describe('SharedDataService', () => {
  ...
  describe('addInningToLog()', ()=> {
    ...
    it('should add a new inning to the log', ()=> {
      ...
      const sampleTurn = {ballsSunk: [], deadBalls:[], name: 'Bob', timeouts: 0} as ITurn;
  ...
  describe('decrementInning()', ()=> {
    ...
    describe('when 2 or more innings in the current game with no balls sunk in them', ()=>{
      ...
      describe('then with 2 more innings with some balls sunk in them', ()=> {
        beforeEach(()=> {
          const newInning: IInning = {lagWinnerTurn: {ballsSunk: [1,2], deadBalls:[], name: 'Bob', timeouts: 0}, lagLoserTurn: {ballsSunk: [3,4], deadBalls:[7], name: 'Mary', timeouts: 0}};
          service.addInningToLog(newInning);
          const newInning2: IInning = {lagWinnerTurn: {ballsSunk: [5], deadBalls:[8], name: 'Bob', timeouts: 0}, lagLoserTurn: {ballsSunk: [6], deadBalls:[], name: 'Mary', timeouts: 0}};
    ...
    describe('when 2 or more innings in the current game with some balls sunk in them (and no blank innings)', ()=> {
      beforeEach(()=> {
        const newInning: IInning = {lagWinnerTurn: {ballsSunk: [1,2], deadBalls:[], name: 'Bob', timeouts: 1}, lagLoserTurn: {ballsSunk: [3,4], deadBalls:[7], name: 'Mary', timeouts: 1}};
        service.addInningToLog(newInning);
        const newInning2: IInning = {lagWinnerTurn: {ballsSunk: [5], deadBalls:[8], name: 'Bob', timeouts: 0}, lagLoserTurn: {ballsSunk: [6], deadBalls:[], name: 'Mary', timeouts: 1}};
        service.addInningToLog(newInning2);
      })
      it('should merge the last 2 innings together', ()=> {
        ...
        const combinedInning = {lagWinnerTurn: {ballsSunk: [1,2,5], deadBalls: [8], name: 'Bob', timeouts:1}, lagLoserTurn: {ballsSunk: [3,4,6], deadBalls:[7], name: 'Mary', timeouts: 2}} as IInning
  ...
  describe('addDeadBall(ballNum)', ()=>{
    it('should add the number into the deadball list of the current shooter', ()=>{
      const startInning = { lagWinnerTurn: {ballsSunk: [], deadBalls: [], name: 'Bob', timeouts: 0} as ITurn } as IInning
